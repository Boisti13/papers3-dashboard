 ...
 * g_dirty.  The actual panel commit only happens inside epd_driver_tick(),
 * which enforces a minimum interval between physical refreshes.
 *
 * Normal updates use epd_fast (full screen, one clean pass, no ghosting).
 * epd_quality is reserved for post-sleep via epd_driver_full_refresh().
 */
...
// ─── Refresh state ────────────────────────────────────────────────────────────
static const uint32_t EPD_MIN_REFRESH_MS  = 1000;
static const uint32_t FULL_REFRESH_EVERY  = 5;
...
void epd_driver_tick() {
    ...
    if (++g_refresh_count >= FULL_REFRESH_EVERY) {
        g_refresh_count = 0;
